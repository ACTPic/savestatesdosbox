...
void POD_Save_Gameblaster( std::ostream& stream )
{
	...
	// - pure data (?) - untested since MAME SAA1099 is used
        WRITE_POD( &device, device );
//	WRITE_POD( &sampleRate, sampleRate );
...
void POD_Load_Gameblaster( std::istream& stream )
{
	...
	// - pure data (?) - untested since MAME SAA1099 is used
        READ_POD( &device, device );
//	READ_POD( &sampleRate, sampleRate );
